...
class CropPlanning(Annealer):
    def __init__(self, state, wj_data, Sk_data, Pk_data, Ck_data):
        ...
        for i in I_values:  # 遍历每一年
            for j in J_values:  # 遍历每块地
                ...
                for k in K_values:
                    # 根据年份的奇偶性选择销售单价
                    if i % 2 == 0:  # 偶数年份
                        price = [8, 6.75, 6.5, 3.75, 6.25, 5.5, 5.75, 5.25, 5.5, 6.25, 5, 5.75, 7, 5.25, 7.25, 4.5, 4.5, 4][k]
                    else:  # 奇数年份
                        price = [9.6, 8.1, 7.8, 4.5, 7.5, 6.6, 6.9, 6.8, 6.6, 7.8, 6, 6.9, 8.4, 6.3, 8.7, 5.4, 5.4, 4.8][k]

                    # 种植成本
                    cost = self.Pk_data[k]  # 假设种植成本已经在初始化时设置

                    # 收益
                    revenue = min(y_ik[i][k], self.Sk_data[k]) * price  # 收益
                    # 成本
                    planting_cost = r_ik[k] * cost  # 种植成本
                    # 利润
...
# 创建问题实例
crop_planning = CropPlanning(state, wj_data, Sk_data, Pk_data, Ck_data)  # 确保传递所有参数
